...
struct clientinfo
{
  uint8_t bssid[ETH_MAC_LEN]; // Transmission MAC
  uint8_t station[ETH_MAC_LEN]; // Source MAC
  uint8_t ap[ETH_MAC_LEN]; // MAC of connected AP 
  int channel;
  int err;
  signed rssi;
};
...
struct clientinfo parse_data(uint8_t *frame, uint16_t framelen, signed rssi, unsigned channel) {
  ...
  ci.err = 0;
  ci.rssi = rssi;
  uint8_t *bssid; // transmitter MAC
  uint8_t *station; // Source MAC
  uint8_t *ap; // Destination MAC
  uint8_t ds;
